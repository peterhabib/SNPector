...
for i,record in enumerate(Ref_File):
    ...
    cg.fill_between(5.5, AMP.iloc[:100, :], start='Start', end='End', score='frequency', scale=9.0, facecolor='red',
                    alpha=1)
    cg.fill_between(5.5, DEL.iloc[:100, :], start='Start', end='End', score='frequency', scale=9.0, facecolor='blue',
                    alpha=1)
    ...
    for line in file:
        ...
        print(line[1],line[2])
        cg.draw_link(5.5, ['chr%s'%int(line[1]), 'chr%s'%int(chrNum)], [int(line[2]), int(position)], [int(line[3]), int(position)], color='Red', alpha=0.03)
